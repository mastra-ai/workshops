...
// ===== Keyboard navigation =====
// Supports:
//   → / ←          : sequential prev/next slide
//   Shift+→ / Shift+← : jump to next/prev group header
function initKeyNav(prevUrl, nextUrl, nextGroupUrl, prevGroupUrl) {
  const sections = Array.from(document.querySelectorAll('.section'));
  const lastSection = sections.length > 0 ? sections[sections.length - 1] : null;

  // Inject "Next →" at the bottom of the last section
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowRight') {
      e.preventDefault();
      if (e.shiftKey && nextGroupUrl) {
        window.location.href = nextGroupUrl;
      } else if (!e.shiftKey && nextUrl) {
    ...
    if (e.key === 'ArrowLeft') {
      e.preventDefault();
      if (e.shiftKey && prevGroupUrl) {
        window.location.href = prevGroupUrl;
      } else if (!e.shiftKey && prevUrl) {
...
function initDevReload() {
  if (!window.__DECKFORGE_DEV__) return;
  try {
    const protocol = location.protocol === 'https:' ? 'wss' : 'ws';
    const ws = new WebSocket(`${protocol}://${location.hostname}:${parseInt(location.port) + 1}`);
    ws.onmessage = (e) => {
      if (e.data === 'reload') {
        location.reload();
      }
    ...
    ws.onclose = () => {
      ...
      setTimeout(initDevReload, 2000);
    };
  } catch (err) {
    // Dev reload not available, that's fine
  }
}
