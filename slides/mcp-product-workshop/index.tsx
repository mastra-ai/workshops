...
const Timeline: Page = () => <Canvas>
  ...
  <div style={{ position: 'relative', marginTop: 140, display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 24 }}>
    <div style={{ position: 'absolute', left: 0, right: 0, top: 61, height: 3, background: c.line }} />
    {[
      { date: 'NOV 2024', event: 'MCP launches', detail: 'Anthropic introduces an open standard for connecting AI to tools and data.', ours: false },
      { date: 'MAR 2025', event: 'Remote servers', detail: 'Streamable HTTP gives MCP a new way to run over the network.', ours: false },
      { date: '12 JUN 2025', event: 'Build an MCP Server with Mastra', detail: 'Our first MCP workshop.', ours: true },
      { date: '18 JUN 2025', event: 'Richer tools', detail: 'Structured outputs and elicitation: return typed data and ask users for input.', ours: false },
      { date: '04 SEP 2025', event: 'Building another MCP server with the new spec', detail: 'Prompts, elicitation and subscriptions.', ours: true },
      { date: 'DEC 2025', event: 'Shared stewardship', detail: 'MCP joins the Linux Foundation’s Agentic AI Foundation.', ours: false },
      { date: 'JUL 2026', event: 'Sessionless MCP', detail: 'Requests carry their own context. Servers no longer need a protocol session.', ours: false },
...
const Practices: Page = () => <Canvas>
  ...
  <div style={{ marginTop: 55 }}>{['Reuse your business logic', 'Keep tools focused', 'Authenticate and authorize', 'Make writes safe to retry', 'Test in the real client'].map((text, i) => <div key={text} style={{ display: 'flex', alignItems: 'center', gap: 42, padding: '24px 0', borderBottom: `1px solid ${c.line}` }}><span style={{ color: c.green, fontFamily: mono, fontSize: 30 }}>0{i + 1}</span><span style={{ fontSize: 45 }}>{text}</span></div>)}</div>
</Canvas>;
...
export const notes: (string | undefined)[] = [
  'Eleven slides: Daniel’s ten sections, plus one session diagram. Keep explanations and caveats spoken rather than adding footnotes. Introduce Daniel Lew and Alex Booker, the workshop promise and two demos.',
  ...
  'Both Mastra workshops were called Build an MCP Server with Mastra, on June 12, 2025 and September 4, 2025 (dates/title supplied by Daniel). Selected MCP milestones: November 2024 launch, March 2025 Streamable HTTP, December 9, 2025 AAIF donation, July 28, 2026 stateless revision. June 18, 2025 also added structured outputs and elicitation; mention only if helpful. Primary references in docs/research.md. The schema date 2024-11-05 is not the public announcement date.',
  'Discuss use cases aloud: return an order, update a CRM account, find an issue. Your embedded agent owns UI, instructions, model choices and outcome; an MCP server lets users bring their familiar assistant. Agents have quirks like coworkers, so one less assistant to learn can lower adoption friction. Team expertise and user preferences determine the right choice. MCP can narrow what you maintain, but tool quality, compatibility, authorization and evaluations remain yours. Daniel’s blame-the-agent point is about ownership of the reasoning experience, not escaping responsibility for broken tools. Both can reuse domain logic. A CLI remains useful where a shell is available; do not turn this into the organizing debate.',
  'Short real ChatGPT demo: ask Can I return order ORD-001?, inspect selected tools, inputs and grounded answer. Requires an approved reachable HTTPS endpoint and verified account/auth/protocol support. Current local bearer-token tests do not prove ChatGPT compatibility. Do not disable authorization or silently publish the mutation server. Rehearsal remains pending; FACILITATOR.md records prerequisites and the explicitly labeled Inspector fallback.',
  ...
  'Explain the picture: previously a stateful server could need information from the initial connection to understand later calls, so requests returned to the instance remembering it. The latest spec carries protocol version/capabilities with each request, removing that dependency. Benefits: any replica, simpler restarts, no protocol-session cleanup. Also useful: no mandatory initialization round trip when the client already knows the version/capabilities, and per-call capability/logging preferences. These are spoken examples rather than more diagram labels. Your application still stores orders/users normally; any replica must have access to required business state and authenticate/authorize the request. Older implementations could already use stateless modes: Before illustrates the stateful case, not every old server. This is about protocol-session state, not OAuth login sessions or conversation memory. Source: SEP-2575, especially lines 35–84. SEP-2567 adds the inconsistent lifetime/sharing rationale; keep basket handles and client lifetime examples for questions, not the initial explanation. Source checkout commit 1dc2c726.',
  'Spec maturing: smaller core, extension mechanism, governance and deprecation policy; not finished or free from breaking changes. Usage growing: the supplied OpenAI-gateway chart. Enterprise adoption: concrete vendor products and workspace integration controls (GitHub, Notion, ChatGPT), plus AAIF investment; do not infer a percentage of enterprise production use. Do not call MCP the longest-living AI concept. Sources: docs/research.md, including Anthropic’s AAIF announcement, GitHub July-spec support and OpenAI custom-app docs.',
  'Give examples verbally: reuse domain rules; describe a narrow tool’s purpose, inputs, outputs and recovery; authenticate with appropriate OAuth/token validation and authorize each tenant/resource operation; confirmation plus idempotency for writes; test tool selection, malformed input, wrong tenant, prompt injection, cancellation and retries in the intended host. Authentication is not business authorization. Do not pass arbitrary upstream tokens through. Handles identify resources, not permission. Source: Anthropic Writing effective tools for agents; local spec authorization/security references in docs/research.md.',
  'Longer demo and questions: show the existing token boundary, identity mapping, domain authorization, tools/resources/workflow and failure drills. The slide means walk through adding auth to the capability boundary, not that a production OAuth server has been implemented here. Label local bearer-token teaching auth versus OAuth. Show missing token, wrong tenant, valid return and duplicate retry. Modern wire proof stays available. FACILITATOR.md has exact commands and fallbacks. No separate closing slide: finish in the code or with questions.',
];
