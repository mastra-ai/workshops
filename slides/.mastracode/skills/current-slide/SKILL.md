...
```
slides/node_modules/.open-slide/current.json
```

Path is relative to the repository root. The OpenSlide package and its `node_modules/` live under `slides/`. Use the `Read` tool. The file is JSON.
...
- `pagePath` — relative to the OpenSlide project. Prefix it with `slides/` before passing it to `Read` or editing the file.
...
1. Read `slides/node_modules/.open-slide/current.json`.
2. Check `updatedAt` is recent.
3. Prefix `pagePath` with `slides/` and read the result (e.g. `slides/q2-roadmap/index.tsx`).
