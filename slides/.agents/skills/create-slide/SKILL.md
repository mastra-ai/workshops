...
Repository layout override: theme paths in this skill are relative to the OpenSlide project, so `themes/` means `slides/themes/` from the repository root. Slide paths remain `slides/<id>/`.
